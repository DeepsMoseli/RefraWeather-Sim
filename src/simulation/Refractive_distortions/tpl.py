...
def apply_tpl_distortion(
    ...
    """
    Warp with Stable(alpha=exponent) SRF on coarse grid then upsampled.
    ...
    model_small = gs.Stable(dim=2, var=var, len_scale=len_scale / downscale, alpha=exponent)
    srf = gs.SRF(model_small, mean=0.0)

    dx_small = srf.structured([np.arange(h_small), np.arange(w_small)], seed=seed_x)
    dy_small = srf.structured([np.arange(h_small), np.arange(w_small)], seed=seed_y)
